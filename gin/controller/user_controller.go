...
func (uc UserController) GetUsers(c *gin.Context) {
	...
	response.Success(c, gin.H{"rows": dto.ToUsersDto(users), "total": total}, "获取用户列表成功")
}
...
func (uc UserController) ResetPwd(c *gin.Context)  {
	...
	if err != nil {
		response.Fail(c, nil, "用户信息查找失败"+err.Error())
		return
	}

	newPwd := util.GenPasswd("123456")

	user.Password = newPwd

	err = uc.UserRepository.ResetPwd(&user)
	if err != nil {
		response.Fail(c, nil, "更新用户信息失败"+err.Error())
		return
	}
